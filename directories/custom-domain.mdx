...
<Tabs>
  <Tab title="Option 1: Nameservers (Recommended)">
    ...
    **Update your nameservers at your domain registrar to:**
    ...
    <Info>
      **Your email keeps working.** Before you switch, the wizard lists the MX, SPF, DKIM, DMARC and mail records it found on your domain. They are copied into the new zone automatically when you submit, and you can edit them any time under **Settings → Manage DNS Records**.
    </Info>
...
Click **Submit** as soon as you have made the change at your DNS provider (or even before — we check either way). From then on the Custom Domain page shows a **live status card** instead of the wizard:

- Each requirement is listed with what we **expect** and what we currently **see** in DNS — nameservers or CNAME records, then the Cloudflare hostname and SSL certificate.
- The **Next step** line tells you the one thing to change. When we can tell which registrar hosts your DNS (GoDaddy, Namecheap, Cloudflare, Squarespace, Wix, IONOS and others), it links to that registrar's own guide.
- We re-check every **5 minutes** and the page refreshes on its own. **Check now** re-reads DNS immediately.
- **Change domain or method** reopens the wizard; **Remove Domain** stops the setup.
...
You'll receive an email when the domain is active. If it is still pending after a day, we email you what we see and what to change; after three days we send one last reminder and then keep checking quietly. After a week without any change we pause the automatic checks — press **Check now** to resume them.
