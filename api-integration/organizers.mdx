...
- `logo` (string, optional): Path to an already-uploaded logo image (max 255 characters)
- `logo_url` (string, optional): URL of an externally hosted logo image (max 255 characters). Use this instead of `logo` to set a logo by URL
- `cover_image` (string, optional): Path to an already-uploaded cover image (max 255 characters)
- `cover_image_url` (string, optional): URL of an externally hosted cover image (max 255 characters). Use this instead of `cover_image` to set a cover by URL
